...
export const useGameStore = create((set, get) => ({
  ...
  get connectedPeers() {
    return Object.entries(get().peers)
      .filter(([, p]) => p.connected)
      .map(([id, p]) => ({ id, ...p }));
  },

  get allReady() {
    const { myReady, peers } = get();
    const connected = Object.values(peers).filter((p) => p.connected);
    return myReady && connected.length > 0 && connected.every((p) => p.ready);
  },

  // Ranked leaderboard (self + connected peers)
  get leaderboard() {
    const { selfId, selfName, myReps, peers } = get();
    const rows = [
      { id: selfId, name: selfName || 'You', reps: myReps, isSelf: true },

...
export function useRoom({ code, selfId, selfName }) {
  ...
  function handleSignalMessage(msg) {
    ...
    roomDbg(`signal msg from ${from?.slice(0,6)} type=${description?.type ?? (candidate ? 'ice' : 'unknown')}`);
  ...
  function handlePeerList(peerObjs) {
    ...
    roomDbg(`peerList [${peerIds.map(id => id.slice(0,6)).join(', ')}]`);

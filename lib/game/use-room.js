...
export function useRoom({ code, selfId, selfName }) {
  ...
  function handlePeerList(peerObjs) {
    ...
    // Ensure we have a connection object for every known peer.
    // The impolite peer (higher ID) will trigger the offer.
  ...
  useEffect(() => {
    if (!store.allReady || store.phase !== PHASE.LOBBY) return;
  ...
  }, [store.allReady, store.phase, selfId]);
